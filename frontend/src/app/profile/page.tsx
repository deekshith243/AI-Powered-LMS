...
import { BookOpen, AlertCircle, Sparkles, Download, BarChart2, Award, Zap, Eye } from 'lucide-react';
...
export default function Profile() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* AI Recommendations */}
      <div className="mt-16 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100 p-8 shadow-sm">
         <h3 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               {recommendations.map(rec => (
                  <Link href={`/learn/${rec.id}`} key={rec.id}>
                     <div className="bg-white rounded-xl shadow-sm border border-indigo-100 p-6 flex flex-col h-full hover:shadow-md transition">
                        <h4 className="text-lg font-bold text-gray-900">{rec.title}</h4>
                        <p className="text-sm text-gray-500 mt-2 line-clamp-2 mb-4 flex-1">{rec.description}</p>
                        <span className="text-sm font-semibold text-indigo-600">Explore Course &rarr;</span>
                     </div>
                  </Link>
               ))}
            </div>
